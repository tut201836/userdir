...
def generate_choreonoid_body_and_spring_settings(x_joint_count=10, z_joint_count=10, joint_width=0.2, floor_type = 0, body_filename="step_floor.body", spring_filename="SpringCustomizerSettings.yaml"):
    # Bodyファイルの基本構造
    body_data = {
    ...
    for i in range(x_joint_count):
        for j in range(z_joint_count):
            ...
            # 沈み込みのみ
            if floor_type == 1 or floor_type == 2:
                    joint_type = "prismatic"
                    joint_axis = [0, 1, 0]  # y軸方向に沈む
                    joint_range = [-0.1, 0.1]  # 上下方向の範囲
            ...
            if floor_type == 2:
                spring_setting = {
                    "joint_name": joint_name,
                    "K": random.randint(2000, 2000),  # 1000から3000のランダムな整数
                    "D": random.randint(2000, 2000),  # 1000から3000のランダムな整数
...
if __name__ == "__main__":
    generate_choreonoid_body_and_spring_settings(x_joint_count=10, z_joint_count=5, joint_width=0.3, floor_type = 2)
